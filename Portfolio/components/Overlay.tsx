...
export default function Overlay() {
    return (
        <div className="absolute top-0 left-0 w-full h-[500vh] pointer-events-none z-10 flex flex-col justify-between pb-[100vh]">
            {/* spacer */}
            {/* Section 1 - Intro */}
            <motion.div
            ...
            {/* Section 2 */}
            <motion.div
                className="h-screen flex items-center justify-start px-8 md:px-20"
            ...
            {/* Section 3 */}
            <motion.div
                className="h-screen flex items-center justify-end px-8 md:px-20"
                initial={{ opacity: 0, x: 50 }}
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.8, ease: "easeOut" }}
                viewport={{ amount: 0.5 }}
            >
                <h2 className="text-5xl md:text-7xl font-bold max-w-4xl text-right leading-tight text-white">
                    Bridging <br />
                    <span className="bg-gradient-to-r from-emerald-400 via-teal-500 to-cyan-500 bg-clip-text text-transparent">design</span> & engineering.
                </h2>
            </motion.div>
        </div>
    );
